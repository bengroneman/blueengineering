...
${validate_component(Hero, "Hero").$$render($$result, {}, {}, {
    cta: () => {
      return `<button slot="${"cta"}" type="${"button"}" class="${"btn-primary mt-8"}"><a href="${"#projects"}" class="${"font-semibold"}" aria-labelledby="${"link to scroll to project section"}">Learn More</a></button>`;
    },
    content: () => {
      return `<div slot="${"content"}"><p class="${"mx-auto"}">I started Blue engineeing to help build community centered around technology<br>
			I work and live out of my home base in Lander Wyoming.<br>
    ...
    title: () => {
      return `<h2 slot="${"title"}" class="${"pt-6"}">Blue Engineering.</h2>`;
    }
  })}
...
<div class="${"relative py-16 bg-white overflow-hidden"}"><div class="${"hidden lg:block lg:absolute lg:inset-y-0 lg:h-full lg:w-full"}"><div class="${"relative h-full text-lg max-w-prose mx-auto"}" aria-hidden="${"true"}"><svg class="${"absolute top-12 left-full transform translate-x-32"}" width="${"404"}" height="${"384"}" fill="${"none"}" viewBox="${"0 0 404 384"}"><defs><pattern id="${"74b3fd99-0a6f-4271-bef2-e80eeafdf357"}" x="${"0"}" y="${"0"}" width="${"20"}" height="${"20"}" patternUnits="${"userSpaceOnUse"}"><rect x="${"0"}" y="${"0"}" width="${"4"}" height="${"4"}" class="${"text-gray-200"}" fill="${"currentColor"}"></rect></pattern></defs><rect width="${"404"}" height="${"384"}" fill="${"url(#74b3fd99-0a6f-4271-bef2-e80eeafdf357)"}"></rect></svg>
	...
	<div class="${"relative px-4 sm:px-6 lg:px-8"}"><div class="${"text-lg max-w-prose mx-auto"}"><h1><span class="${"block text-base text-center text-indigo-600 font-semibold tracking-wide uppercase"}">About</span>
				<span class="${"mt-2 block text-3xl text-center leading-8 font-extrabold tracking-tight text-gray-900 sm:text-4xl"}">Blue Engineering</span></h1>
			<p class="${"mt-8 text-xl text-gray-500 leading-8"}">Aliquet nec orci mattis amet quisque ullamcorper neque, nibh sem. At arcu, sit dui mi, nibh
		...
		<div class="${"mt-6 prose prose-indigo prose-lg text-gray-500 mx-auto"}"><p>Faucibus commodo massa rhoncus, volutpat. <strong>Dignissim</strong> sed
			...
			<ul role="${"list"}"><li>Quis elit egestas venenatis mattis dignissim.</li>
			...
			<blockquote><p>Sagittis scelerisque nulla cursus in enim consectetur quam. Dictum urna sed consectetur
			...
			<figure><img class="${"w-full rounded-lg"}" src="${"https://images.unsplash.com/photo-1500648767791-00dcc994a43e?ixlib=rb-1.2.1&auto=format&fit=facearea&w=1310&h=873&q=80&facepad=3"}" alt="${""}" width="${"1310"}" height="${"873"}">
				<figcaption>Sagittis scelerisque nulla cursus in enim consectetur quam.</figcaption></figure>
